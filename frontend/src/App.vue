<template>
  ...
  <main>
    <header><div><h1>多源异构数据对齐与融合</h1><p>温度图片、振动、音频与视频数据的一体化可视分析</p></div><div class="header-actions"><span class="dataset-pill">Multi-source Dataset</span><a id="export" class="icon-btn" title="导出结果">⇩</a></div></header>

    <section class="stats">
      <div><span>数据样本</span><strong id="totalSamples">—</strong><small>组同步记录</small></div>
      <div><span>数据模态</span><strong id="modalities">4</strong><small>异构传感源</small></div>
      <div><span>活动分组</span><strong id="groupCount">7</strong><small>行为类别</small></div>
      <div><span>当前状态</span><strong class="status-text" id="globalStatus">待处理</strong><small id="statusHint">请选择数据组</small></div>
    </section>

    <section class="panel selection-panel">
      <div class="panel-title"><div><span class="step">01</span><div><h2>选择数据组</h2><p>温度图片、振动数据、音频数据和视频数据四类同步记录</p></div></div><label>具体样本 <select id="sampleSelect"></select></label></div>
    ...
    <section class="panel process-panel" id="alignPanel">
      <div class="panel-title"><div><span class="step purple">02</span><div><h2>时空对齐</h2><p>基于统一时间轴的多模态最近邻匹配</p></div></div><button class="primary" id="alignBtn">开始对齐 <span>→</span></button></div>
      <div class="pipeline">
    ...
    <section class="panel process-panel disabled" id="fusionPanel">
      <div class="panel-title"><div><span class="step cyan">03</span><div><h2>多模态融合</h2><p>特征归一化与自适应加权融合</p></div></div><button class="primary cyan-btn" id="fuseBtn" disabled>开始融合 <span>→</span></button></div>
      ...
      <div class="results hidden" id="fusionResults">
        <div class="decision-card" id="decisionCard">
          ...
          <div class="decision-detail"><div><span>融合得分</span><b id="decisionScore">—</b></div><div><span>融合置信度</span><b id="decisionConfidence">—</b></div><div><span>主要贡献模态</span><b id="decisionSources">—</b></div><div><span>时空对齐结果</span><b id="decisionAlignment">—</b></div></div>
          ...
          <p id="decisionNote">主要贡献模态由各模态异常得分与融合权重共同决定，并非仅由权重决定。</p>
        </div>
        <div class="fusion-summary core-summary"><div><span>对齐前 RMSE</span><strong id="coreBeforeRmse">—</strong><small>越小越好</small></div><div><span>对齐后 RMSE</span><strong id="coreAfterRmse">—</strong><small>越小越好</small></div><div><span>融合得分</span><strong id="coreFusionScore">—</strong><small>用于三档判别</small></div><div><span>融合置信度</span><strong id="coreConfidence">—</strong><small>越大响应越强</small></div><div><span>主要贡献模态</span><strong id="coreSource">—</strong><small>权重 × 异常得分</small></div><div><span>对齐成功率</span><strong id="coreSuccessRate">—</strong><small>越大越好</small></div></div>
        <div class="fusion-grid"><div class="chart-card"><h3>融合特征时序 <small id="fusionAxisLabel">真实时间（s）</small></h3><canvas id="fusionChart"></canvas></div><div class="chart-card"><h3>模态贡献占比</h3><canvas id="donutChart"></canvas><div id="donutLegend" class="donut-legend"></div></div></div>
        <div class="result-head contribution-head"><h3>模态贡献明细</h3><small>贡献值 = 融合权重 × 模态异常得分</small></div>
        <div class="contribution-table"><div class="contribution-row table-header"><span>模态名称</span><span>权重</span><span>异常得分</span><span>贡献值</span></div><div id="contributionRows"></div></div>
